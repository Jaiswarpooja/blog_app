...
function Post({post={ id:-1 ,title:"This is default post title",content:"THis is default post content"}, deletedPost }){
        ...
        <CardBody>
            ...
            <div>
                <Link className="btn btn-secondary" to={"/posts/"+post.postId}>Read more</Link>
                 ...
                 {
                    userContextData.user.login && (user && user?.id === post?.user?.id ? <Button onClick={(event)=>deletedPost(post)} color="danger" className="ms-2">Delete</Button>:'')
                 }
